...
func TestSeries(t *testing.T)      { benchSeries(testutil.NewTB(t)) }
func BenchmarkSeries(b *testing.B) { benchSeries(testutil.NewTB(b)) }

func benchSeries(t testutil.TB) {
	...
	const numSeriesPerBlock = 250000 // 4 blocks will make 1mln.
	...
	// Huge chunk Pool - always reuse. Let's compare.
	chunkPool, err := pool.NewBytesPool(maxChunkSize, 50e6, 2, 50e6)
	testutil.Ok(t, err)
	...
	for bi := 0; bi < 4; bi++ {
		...
		for i := 0; i < numSeriesPerBlock; i++ {
			...
			// Put ASCII representation of the number instead of number to have sorted behaviour.
			lbls := labels.FromStrings("foo", "bar", "i", fmt.Sprintf("%s%s", string(ts), postingsBenchSuffix))
	...
	if t.IsBenchmark() {
		...
		// For test just go straight away to the largest one, we care here only about correctness for 1 mln series.
		s := 4 * numSeriesPerBlock
